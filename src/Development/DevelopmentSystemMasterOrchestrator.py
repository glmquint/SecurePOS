...
class DevelopmentSystemMasterOrchestrator:
    ...
    def __init__(self, status: DevelopmentSystemStatus = None):
        if status is None:
            self.status = DevelopmentSystemStatus(f"{os.path.dirname(__file__)}/development_system_status.json", f"{os.path.dirname(__file__)}/schema/status_schema.json")
            self.status.load_status()
        else:
            self.status = status
        self.development_system_configurations = DevelopmentSystemConfigurations(f'{os.path.dirname(__file__)}/schema/config_schema.json')
