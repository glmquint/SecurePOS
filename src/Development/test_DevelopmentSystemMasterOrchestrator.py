...
class TestDevelopmentSystemMasterOrchestrator(TestCase):
    def test_start(self):
        status = DevelopmentSystemStatus(f"{os.path.dirname(__file__)}/development_system_status.json", f"{os.path.dirname(__file__)}/schema/status_schema.json")
        status.load_status()
        ...
        ds.start()
        assert (ds.status.status == "send_to_production" or ds.status.status == "send_to_messaging")
