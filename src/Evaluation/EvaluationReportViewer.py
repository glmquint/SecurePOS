    ...
    def save_evaluation_result(self, modelcontroller, tick_array):
        ...
        for x in range(1, modelcontroller.sufficient_label_number + 1):
            ...
            if x <= self.row_offset:
                ...
                imgDraw.text((self.first_label_x_offset, self.y_offset(x)), str(labels[x - 1].label), font=font,
                             fill=self.black)
                imgDraw.text((self.second_label_x_offset, self.y_offset(x)), security_labels[x - 1].label, font=font,
                             fill=self.black)
            ...
            # second row
                ...
                y = x - self.row_offset
                imgDraw.text((self.row_id_x_offset + self.second_column_offset, self.y_offset(y)), str(x) + ")",
                             font=font, fill=(0, 0, 0))
                imgDraw.text((self.first_label_x_offset + self.second_column_offset, self.y_offset(y)),
                             labels[x - 1].label, font=font, fill=(0, 0, 0))
                imgDraw.text((self.second_label_x_offset + self.second_column_offset, self.y_offset(y)),
                             security_labels[x - 1].label, font=font, fill=(0, 0, 0))
                imgDraw.rectangle([(self.x0_rect + self.second_column_offset, self.y_offset(y)), (
                    self.x0_rect + self.x1_rect_offset + self.second_column_offset,
                    self.y_offset(y) + self.x1_rect_offset)], None, "black")
                if tick_array[x - 1] == "V":
                    tick_color = self.green
                else:
                    tick_color = self.red
                imgDraw.text((self.tick_x_offset + self.second_column_offset, self.y_offset(y)), tick_array[x - 1],
                             font=font, fill=tick_color)
            # third row
            elif x <= 50:
                y = x - self.row_offset * 2
                imgDraw.text((self.row_id_x_offset + self.third_column_offset, self.y_offset(y)), str(x) + ")",
                             font=font, fill=self.black)
                imgDraw.text((self.first_label_x_offset + self.third_column_offset, self.y_offset(y)),
                             labels[x - 1].label, font=font, fill=self.black)
                imgDraw.text([self.second_label_x_offset + self.third_column_offset, self.y_offset(y)],
                             " " + security_labels[x - 1].label, font=font, fill=self.black)
                imgDraw.rectangle([(self.x0_rect + self.third_column_offset, self.y_offset(y)), (
                    self.x0_rect + self.x1_rect_offset + self.third_column_offset,
                    self.y_offset(y) + self.x1_rect_offset)], None, "black")
                if tick_array[x - 1] == "V":
                    tick_color = self.green
                else:
                    tick_color = self.red
                imgDraw.text((self.tick_x_offset + self.third_column_offset, self.y_offset(y)), tick_array[x - 1],
                             font=font, fill=tick_color)
        ...
        imgDraw.line([(self.firstcorner, self.secondcorner), (self.firstcorner, self.height)], fill=(self.black),
                     width=self.widthline)
        imgDraw.line([(self.firstcorner, self.secondcorner), (self.width, self.secondcorner)], fill=(self.black),
                     width=self.widthline)
