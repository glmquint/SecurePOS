...
class ProductionSystemOrchestrator:
    ...
    def __init__(self, config: ProductionSystemConfig = None):
        ...
        self.phase_tracker = PhaseTracker({"production_phase_duration": self.production_config.monitoring_window,
        ...
        self.prod_sys_receiver = ProductionSystemReceiver(self.production_config.server_port, self.system_bus)
        self.sender = ProductionSystemSender(self.production_config.message_url,
    ...
    def main(self):
        ...
        thread.start()
        attack_risk_classifier = AttackRiskClassifier(self.system_bus, self.classifier)
        while True:
            # if the classifier.sav is not present try to pop it from the system_bus for synchronisation
